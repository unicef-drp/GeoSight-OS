...
class StylePermissionTest(BasePermissionTest.TestCase):
    ...
    def setUp(self):
        ...
        self.resource_1 = Style.permissions.create(
            user=self.resource_creator,
            name='Name A',
            group='Group 1'
        )
        self.resource_2 = Style.permissions.create(
            user=self.resource_creator,
            name='Name B',
            group='Group 2',
            description='This is test'
        )
        self.resource_3 = Style.permissions.create(
            user=self.resource_creator,
            name='Name C',
            group='Group 3',
    ...
    def test_detail_api(self):
        """Test GET DETAIL API."""
        url = reverse('styles-detail', args=[self.resource_1.id])
        ...
        url = reverse(
            'styles-detail', kwargs={'id': self.resource_3.id}
        )
        self.assertRequestGetView(url, 403)
        self.assertRequestGetView(url, 403, user=self.viewer)
        self.assertRequestGetView(url, 403, user=self.creator)
    ...
    def test_destroy_api(self):
        ...
        self.check_delete_resource_with_different_users(
            id, 'styles-detail')
        self.assertIsNone(Style.objects.filter(id=id).first())
    ...
    def test_delete_api(self):
        """Test DELETE API."""
        resource_1 = Style.permissions.create(
            user=self.resource_creator,
            name='Name AA',
            group='Group 1'
        )
        resource_2 = Style.permissions.create(
            user=self.resource_creator,
            name='Name BA',
            group='Group 2',
            description='This is test'
        )
        resource_3 = Style.permissions.create(
            user=self.resource_creator,
            name='Name CA',
            group='Group 3',
            description='Resource 3'
