...
class BasemapListApiTest(BasePermissionTest, TestCase):
    ...
    def test_delete_api(self):
        ...
        url = reverse('basemap-detail-api', kwargs={'pk': resource.id})
        ...
        response = self.assertRequestGetView(
            reverse('basemap-list-api'), 200, self.creator)
        self.assertEqual(len(response.json()), 1)
    ...
    def test_delete_multiple_api(self):
        ...
        self.assertEqual(BasemapLayer.objects.count(), 2)
        self.assertRequestDeleteView(
            url, 200, self.creator, data={'ids': json.dumps([resource.id])}
        )
        self.assertEqual(BasemapLayer.objects.count(), 1)
