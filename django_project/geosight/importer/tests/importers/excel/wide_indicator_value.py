...
from geosight.importer.importers import IndicatorValueExcelWideFormat
...
class ExcelWideFormatIndicatorValueTest(BaseIndicatorValueImporterTest):
    """Test for Importer : Excel Wide Format Indicator Value."""
    ...
    def test_correct_importer(self):
        """Test if correct importer."""
        self.assertEqual(self.importer.importer, IndicatorValueExcelWideFormat)

    def test_run_error_attributes(self):
        """Test if error attributes importer."""
        filepath = ABS_PATH(
            'geosight', 'importer', 'tests', 'importers',
            '_fixtures', 'excel_wide_indicator_value_error.xlsx'
        )
    ...
    def test_run_error(self):
        ...
        with open(filepath, 'rb') as _file:
            attributes = {
                **self.attributes, **{
                    'mapping': {
                        'IND1': f'indicator-{self.indicator_1.id}',
                        'IND2': f'indicator-{self.indicator_2.id}',
                        'IND3': f'indicator-{self.indicator_3.id}',
                    }
                }
            }
            files = {
    ...
    def test_run(self):
        ...
        filepath = ABS_PATH(
            'geosight', 'importer', 'tests', 'importers',
            '_fixtures', 'excel_wide_indicator_value.xlsx'
        )
    ...
    def test_run_using_limit_by_data(self):
        """Test if correct importer."""
        filepath = ABS_PATH(
            'geosight', 'importer', 'tests', 'importers',
            '_fixtures', 'excel_wide_indicator_value.xlsx'
    ...
    def test_multiple_values_data(self):
        ...
        filepath = ABS_PATH(
            'geosight', 'importer', 'tests', 'importers',
            '_fixtures', 'excel_wide_indicator_value (Multiple Value).xlsx'
        )
    ...
    def test_multiple_values_data_string(self):
        ...
        filepath = ABS_PATH(
            'geosight', 'importer', 'tests', 'importers',
            '_fixtures',
            'excel_wide_indicator_value (Multiple Value String).xlsx'
        )
