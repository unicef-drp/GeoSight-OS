...
from geosight.importer.importers import IndicatorValueSharepointExcelWideFormat
...
class SharepointWideFormatIndicatorValueTest(BaseSharepointIndicatorValueTest):
    """Test for Importer : Sharepoint Wide Format Indicator Value."""
    ...
    def test_correct_importer(self):
        """Test if correct importer."""
        self.assertEqual(
            self.importer.importer, IndicatorValueSharepointExcelWideFormat
        )
        ...
        attributes = {
            **self.attributes, **{
                ...
                'sharepoint_relative_path':
                    'excel_wide_indicator_value_error.xlsx'
            }
        }
    ...
    def test_run(self):
        ...
        attributes = {
            **self.attributes, **{
                'sharepoint_relative_path':
                    'excel_wide_indicator_value.xlsx',
                'mapping': {
                    'IND1': f'indicator-{self.indicator_1.id}',
                    'IND2': f'indicator-{self.indicator_2.id}',
                    'IND3': f'indicator-{self.indicator_3.id}',
