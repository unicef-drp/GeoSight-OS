...
const ArcGisStyle = async (map, id, layer) => {
  ...
  const assignCases = (styleType, propertyType, cases) => {
    ...
    switch (styleType) {
      case "polygon":
        if (["color", "weight"].includes(propertyType)) {
          layerId = lineId;
        } else {
          layerId = fillId;
        }
    ...
    if (layerId) {
      ...
      casesByType[layerId][propertyType] = casesByType[layerId][propertyType]
        ? casesByType[layerId][propertyType]
        : [];
      casesByType[layerId][propertyType] =
        casesByType[layerId][propertyType].concat(cases);
    }
  };
  switch (style.classificationValueMethod) {
    case "noClassification": { // No classification
        ...
        style.classifications.map((classification, idx) => {
          ...
          for (let [key, value] of Object.entries(classStyle)) {
            if (value) {
              ...
              cases.push(["==", ["get", style.fieldName], classValue]);
              cases.push(value);
              if (!isNaN(classValue)) {
            ...
            if (value) {
              ...
              fieldNames.map((fieldName, idx) => {
                ...
                if (!isNaN(classValue) && classValue !== null) {
                  classValue = parseFloat(classValue);
                  valueClassificationNumber.push([
                    "==",
                    ["get", fieldName],
                    classValue,
                  ]);
              ...
              if (valueClassification.length === 1) {
                ...
                cases.push(["all", ...valueClassification]);
                cases.push(value);
              }
              ...
              if (valueClassificationNumber.length === 1) {
                ...
                cases.push(["all", ...valueClassificationNumber]);
                cases.push(value);
              }
              assignCases(classification.style.type, key, cases);
            }
        ...
        switch (paintProperty) {
          ...
          default: {
            ...
            if (cases.length === 1) {
              paint = cases[0];
              if (
                paintProperty.includes("opacity") ||
                paintProperty.includes("width")
              ) {
                paint = toFloat(paint);
              }
            } else if (cases.length) {
