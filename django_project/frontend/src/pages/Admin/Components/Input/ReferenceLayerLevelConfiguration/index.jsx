...
export const ViewLevelConfiguration = forwardRef(
  (
    ...
    const referenceLayerRequest = useSelector(
    ...
    useEffect(() => {
      if (datasetLevels) {
        ...
        let levels = data.levels
          ? datasetLevels
              .filter((datasetLevel) =>
                data.levels.includes(datasetLevel.level),
              )
    ...
    const levels = datasetLevels
      ? datasetLevels.map((level) => level.level_name)
      : [];
    const defaultLevel = datasetLevels?.find(
      (level) => level.level === data.default_level,
    )?.level_name;
    const availableLayers = datasetLevels
      ?.filter((level) => data?.levels?.includes(level.level))
                      ...
                      dataSelected={(selectedData) => {
                        ...
                        if (selectedData[0]) {
                          ...
                          selected = {
                            ...
                            detail_url: selectedData[0].is_local
                              ? InternalReferenceDatasets.centroid(identifier)
          ...
          <Grid item xs={6}>
            <div className="BasicFormSection">
              ...
              <SelectWithSearch
                name="default_admin"
                placeholder={
                  datasetLevels ? "Select default admin level" : "Loading"
                }
                options={levels}
                value={defaultLevel ? defaultLevel : ""}
                onChangeFn={(evt) => {
                  data.default_level = datasetLevels?.find(
                    (level) => level.level_name === evt,
                  )?.level;
                  setData({ ...data });
                }}
          ...
          >
            <div className="BasicFormSection">
              ...
              <MultipleSelectWithSearch
                ...
                onChangeFn={(evt) => {
                  data.levels = datasetLevels
                    ?.filter((level) => evt.includes(level.level_name))
                    .map((level) => level.level);
                  setData({ ...data });
                }}
                fullWidth={true}
