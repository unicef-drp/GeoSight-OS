...
export default function CloudNativeGISStreamUpload(
  ...
  const handleChangeStatus = ({ meta, file, xhr }, status) => {
    if (['done', 'error_upload'].includes(status)) {
      ...
      if (!isNaN(_id)) {
        urls.api.cloudNativeGISUploadFile = urls.api.cloudNativeGISUploadFileMain.replace('0', _id)
        ...
        <div
          ...
          <div className='InputInLine'>
            <Dropzone
              ...
              accept=".zip"
              multiple={false}
              inputContent={'Click to browse for a file in one of these formats: a zipped file containing a shapefile.'}
              PreviewComponent={
                preview => {
                  return <div></div>
                }
