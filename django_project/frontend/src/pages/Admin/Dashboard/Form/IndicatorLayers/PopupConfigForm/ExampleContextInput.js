...
export default function ExampleContextInput({
  ...
  const selectedGeometries = geometries
    ? geometries[levelSelected?.value]
    : null;
        ...
        getContext(
          ...
          function contextOnLoad(context) {
            ...
            currentIndicatorLayer.indicators.map((indicator) => {
              let data =
                context.context.admin_boundary.indicators[indicator.shortcode];
              if (data) {
                data = data[0];
                data.date = data?.time;
                data.indicator = indicator;
                indicatorValueByGeometry[featureProperties.ucode].push({
                  ...indicator,
                  ...data,
                });
              } else {
  ...
  return (
    <div className="indicator-layer context">
      <div className="title">
        <div>
          Example context. Select admin level and the geometry to fetch the
          actual context.
        </div>
        {!datasetLevels ? (
          "Loading"
        ...
        {!geometries || !geometries[levelSelected?.value] ? (
          "Loading"
        ) : (
          <Select
