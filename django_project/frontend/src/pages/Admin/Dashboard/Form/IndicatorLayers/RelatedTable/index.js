...
export default function RelatedTableLayerConfig({
  ...
  const { referenceLayer, geoField } = useSelector(
  ...
  const defaultDescription = (layer, relatedTableConfig) => {
    ...
    let description =
      "Layer dynamically created based on {related-table-name} table. Data has been aggregated " +
  ...
  useEffect(() => {
    ...
    if (relatedTableConfig.geography_code_field_name) {
      params["geography_code_field_name"] =
        relatedTableConfig.geography_code_field_name;
    }
    ...
    if (
      ...
      fetchPagination(url, { ...params, page: 1, page_size: 10000 }).then(
        (response) => {
  ...
  let rawDataPopupConfig =
    data?.raw_data_popup_config?.filter((config) => config.name) || [];
  relatedFields?.map((field) => {
    if (!rawDataPopupConfig.find((config) => config.name === field.name)) {
      rawDataPopupConfig.push({
        name: field.name,
        alias: field.alias,
        type: field.type,
        visible: true,
        order: rawDataPopupConfig.length,
      });
    }
  });
              ...
              forms={{
                General: (
                  <div>
                    {/* STYLE DATA */}
                    {!data.style_type ? (
                      <span className="form-helptext error">
                    ...
                    <div className="BasicFormSection">
                      ...
                      <SelectWithList
                        ...
                        onChange={(evt) => {
                          data.related_tables = relatedTables.filter(
                            (rt) => rt.id === evt.value,
                          );
                          updateData();
                        }}
                    ...
                    <div className="BasicFormSection">
                      ...
                      <SelectWithList
                        list={relatedTableConfig.related_fields.map((field) => {
                          return {
                            name: field,
                            value: field,
                          };
                      ...
                      />
                      <span className="form-helptext">
                        Related field as geography code. Change it on related
                        tables section.
                      </span>
                    </div>
                    <div className="BasicFormSection">
                ...
                "Related Record Popup": (
                  <>
                    <div className="BasicFormSection">
                      <FormControlLabel
                        checked={!!data.raw_data_popup_enable}
                        control={<Checkbox name="raw_data_popup_enable" />}
                        onChange={(evt) => {
                          data.raw_data_popup_enable = evt.target.checked;
                          updateData();
                        }}
                        label={"Enable popup for Related Records"}
                      />
                    </div>
                    <FieldConfig
                      data_fields={rawDataPopupConfig}
                      update={(fields) => {
                        data.raw_data_popup_config = fields;
                        updateData();
                      }}
                    />
                  </>
