 ...
 * @returns {*}
 */
export const cleanWhereQuery = (where) => {
  return where
    .replaceAll('"', "`")
 ...
 * @returns {*}
 */
export const updateWhereQuery = (where, config) => {
  // INTERVAL
  ...
  // INTERNEXT
  {
    ...
    where = where
      ...
      .replace(intervalRegex, function (match, text, href) {
        const clean = match
          .replace(INTERNEXT_QUERY_BEFORE, "")
          .replaceAll("'", "")
          .split(" ");
        const amount = clean[0];
        const interval = clean[1];
 ...
 */
export const getRelatedTableData = (
  data,
  config,
  selectedGlobalTime,
  geoField = "geometry_code",
  aggregateDate = true,
  adminLevels = null,
) => {
  if (data) {
    ...
    // INTERVAL
    let where = updateWhereQuery(config?.where);
    ...
    if (aggregation) {
      ...
      aggregation_full = aggregation_full.replace(
        aggregation_field,
        spacedField(aggregation_field),
      );
    ...
    try {
      if (!date_field) {
        ...
        let sql =
          `SELECT ${geography_code_field_name}             as _geometry_code,` +
          (aggregateDate ? `MAX(data.${date_field})` : `data.${date_field}`) +
          ` as _date,
                          MAX(data.concept_uuid)                   as _concept_uuid,
                          ...
                          MAX(data.admin_level)        as _admin_level
                   FROM ? as data` +
          (updatedWhere ? ` WHERE ${updatedWhere}` : "") +
          ` GROUP BY ${geography_code_field_name}` +
          (!aggregateDate ? `, data.${date_field}` : "") +
          `  
        ...
        if (["MAJORITY", "MINORITY"].includes(aggregation_method)) {
          sql =
            ...
            ` as _date,
                 ...
                 FROM ? as data` +
            (updatedWhere ? ` WHERE ${updatedWhere}` : "") +
            ` GROUP BY ${geography_code_field_name}, data.${aggregation_field}` +
            (!aggregateDate ? `, data.${date_field}` : "") +
 ...
 */
export const buildGeojsonFromRelatedData = (
  data,
  lon_field,
  lat_field,
  query = undefined,
  ...
  let finalData = data;
  if (query) {
    const where = cleanWhereQuery(query);
...
export function getRelatedTableFields(relatedTable, relatedTableData) {
  if (relatedTable?.fields_definition) {
    return relatedTable.fields_definition.map((field) => {
      let options = null;
      if (relatedTableData) {
        options = relatedTableData.map((data) =>
