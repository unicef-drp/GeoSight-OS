...
/**
 * Return indicators with datasets
 */
export function getIndicatorsDatasets(
  currentIndicatorLayer, currentIndicatorSecondLayer, indicatorLayers, indicators, referenceLayer
) {
  // -----------------------------------------
  // GET QUEUE
  // -----------------------------------------
  ...
  [currentIndicatorLayer, currentIndicatorSecondLayer].concat(indicatorLayers).map(layer => {
    if (layer?.indicators) {
      layer?.indicators?.map(_ => {
        ...
        if (layer?.level_config?.referenceLayer?.identifier) {
          ids[_.id].push(layer?.level_config?.referenceLayer?.identifier)
        } else {
          ids[_.id].push(referenceLayer.identifier)
        }
        if (!orderedIds.includes(_.id)) {
          orderedIds.push(_.id)
        }
      })
    }
  });
