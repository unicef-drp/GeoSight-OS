...
export const SELECTABLE_OPERATORS =
  MULTI_SELECTABLE_OPERATORS + SINGLE_SELECTABLE_OPERATORS;
...
export const getOperators = (type, isSimplified) => {
  if (type === "text") {
    return dictDeepCopy(
      isSimplified ? STRING_OPERATORS_SIMPLIFIED : STRING_OPERATORS,
    );
  } else if (type === "date") {
    const operators = dictDeepCopy(
      isSimplified ? DATE_OPERATORS_SIMPLIFIED : DATE_OPERATORS_SIMPLIFIED,
    );
    ...
    return dictDeepCopy(
      isSimplified ? NUMBER_OPERATORS_SIMPLIFIED : NUMBER_OPERATORS,
    );
  }
};
 ...
 */
export function returnWhere(
  where,
  ignoreActive,
  ids,
  sameGroupOperator = true,
  changeGeometryId = true,
) {
  switch (where.type) {
    case TYPE.GROUP:
      ...
      if (queries.length === 0) {
        ...
        if (sameGroupOperator) {
          return `(${queries.join(` ${where.operator} `)})`;
        } else {
          return `(${queries.join(` `)})`;
        }
      }
    case TYPE.EXPRESSION:
      ...
      const used =
        ignoreActive || (where.active && (!ids || ids.includes(fieldSplit[0])));
      return used
        ? returnDataToExpression(field, where.operator, where.value)
        : "";
  }
}
...
export function returnWhereToDict(where, upperWhere) {
  let fromQuery = [];
  if (where) {
    const whereOperator = upperWhere?.operator
      ? upperWhere?.operator
      : WHERE_OPERATOR.AND;
    switch (where.type) {
      ...
      case "AndExpression":
        return []
          .concat(returnWhereToDict(where.left, where))
          .concat(returnWhereToDict(where.right, where));
