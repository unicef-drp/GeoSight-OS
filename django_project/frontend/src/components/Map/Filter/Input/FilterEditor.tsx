...
export function FilterEditorModal({
  ...
  // Operator callbacks
  const setOperatorCallback = (value: string) => {
    let newValue = updateValue(value, data.value);
    if (data.value === newValue) {
      setData({ ...data, operator: value });
    } else {
      setData({ ...data, operator: value, value: newValue });
    }
  };
...
/** Filter editor */
export const FilterEditor = forwardRef(({}, ref) => {
