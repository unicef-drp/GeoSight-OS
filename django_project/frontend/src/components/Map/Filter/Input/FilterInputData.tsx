...
export const FilterInputData = memo(
  ({
    ...
    const updateFilter = () => {
      if (active) {
        const queryWhere = returnDataToExpression(
          `data.${keyField}`,
          operator,
          value,
        );
      ...
      <>
        {sourceDataKey ? (
          <>
            ...
            {sourceDataType === SourceDataType.INDICATOR ? (
              <FetchSourceDetailIndicator id={id} onChange={receiveSource} />
            ) : sourceDataType === SourceDataType.INDICATOR_LAYER ? (
              <FetchSourceDetailIndicatorLayer
                id={id}
              ...
              <FetchSourceDetailRelatedTable id={id} onChange={receiveSource} />
            ) : null}
            ...
            {sourceDataKey !== SourceDataKey.GEOMETRY ? (
              ...
              <FetchSourceGeometryData field={field} onChange={receiveData} />
            )}

            {/* GET OPTIONS DATA*/}
            {sourceDataType === SourceDataType.INDICATOR &&
            keyField === "value" ? (
              <FetchIndicatorOptions id={id} onChange={receiveOptions} />
            ) : sourceDataType === SourceDataType.RELATED_TABLE ? (
              <FetchRelatedTableOptions
                id={id}
                keyField={keyField}
