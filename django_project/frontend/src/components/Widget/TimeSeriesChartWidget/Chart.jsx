...
import { chartAreaBorder } from "../../../contrib/ChartJS";
 ...
 */
export default function Chart({ chartData, selectedLabel, fill = true }) {
  ...
  if (chartData) {
    if (
      ...
      chartData.datasets.filter((dataset) => dataset.data.length === 0).length
    ) {
      empty = true;
    }
  }
        ...
        <div className="widget__time_series__wrapper">
          ...
          <div className="widget__time_series__chart">
            <Line
              ...
              data={{
                ...chartData,
                datasets: chartData.datasets.filter((data) =>
                  selectedLegend.includes(data.label),
                ),
              }}
            />
          </div>
            ...
            {chartData.datasets.map((dataset) => {
              ...
              return (
                <div
                  key={label}
                  className={
                    "widget__time_series__row " + (selected ? "" : "unselected")
                  }
                  onClick={(evt) => {
                    if (selected) {
                      setSelectedLegend(
                        selectedLegend.filter((legend) => legend !== label),
                      );
                    } else {
                ...
                >
                  <div className="widget__time_series__row_inner">
                    <div
                      className="widget__time_series__color"
