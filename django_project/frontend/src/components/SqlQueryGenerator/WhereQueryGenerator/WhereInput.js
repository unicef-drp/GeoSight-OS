...
export function WhereInputValue({
  ...
  useEffect(() => {
    if (isDate && operator === IS_BETWEEN) {
      try {
        const newValue = value;
        let updated = false;
        if (!(value[0]?.length > 3)) {
          newValue[0] = `'${nowUTC(true).toISOString()}'`;
          updated = true;
        }
        if (!(value[1]?.length > 3)) {
          newValue[1] = `'${nowUTC(true).toISOString()}'`;
          updated = true;
        }
        if (updated) {
          setValue(newValue);
  ...
  if (isDate) {
    if ([">", ">=", "<", "<="].includes(operator)) {
      ...
      const min = (
        !(value[0]?.length > 3) ? nowUTC(true).toISOString() : value[0]
      ).replaceAll("'", "");
      const max = (
        !(value[1]?.length > 3) ? nowUTC(true).toISOString() : value[1]
      ).replaceAll("'", "");
      return (
        <LocalizationProvider dateAdapter={AdapterMoment}>
  ...
  if ([IS_NULL, IS_NOT_NULL].includes(operator)) {
    ...
    [OPERATOR_WITH_INTERVAL, OPERATOR_WITH_INTERNEXT].includes(operator)
  ) {
    let timeValue = "";
    let timeType = "";
    const IDENTIFIER =
      operator === OPERATOR_WITH_INTERVAL
        ? INTERVAL_IDENTIFIER
        : INTERNEXT_IDENTIFIER;
    try {
      const [newTimeValue, newTimeType] = value
        .replace(INTERVAL_IDENTIFIER, "")
        .replace(INTERNEXT_IDENTIFIER, "")
        .split(" ");
      timeValue = newTimeValue;
      timeType = newTimeType;
    } catch (err) {}
    return (
      <div className="DateOperator">
        <span className="WhereConfigurationOperatorText">
          {operator === OPERATOR_WITH_INTERVAL ? "Last" : "Next"}
        </span>
        <Input
          type="number"
          className={"WhereConfigurationOperatorValue"}
          ...
          onChange={(evt) => {
            setValue(
              `${evt.target.value && evt.target.value >= 0 ? evt.target.value : 0} ${timeType}${IDENTIFIER}`,
            );
          }}
          disabled={disabled}
        />
    ...
    return (
      <div className="FilterInput MuiInputSliderWithInput MuiInputSliderWithInputMulti">
        <Input
          ...
          onChange={(event) => {
            ...
            setValue([
              val > initBetweenMax ? initBetweenMax : val,
              initBetweenMax,
            ]);
          }}
          inputProps={{
        ...
        <div className="MuiInputSlider">
          <Slider
            ...
            onChangeCommitted={(e) =>
              setValue([initBetweenMin, initBetweenMax])
            }
            disabled={disabled}
          />
        </div>
        <Input
          ...
          onChange={(event) => {
            ...
            setValue([
              initBetweenMin,
              val < initBetweenMin ? initBetweenMin : val,
            ]);
          }}
          inputProps={{
            min: min,
    ...
    if (!isNaN(min) || !isNaN(max)) {
      ...
      return (
        <div className={"MuiInputSliderWithInput " + (isEqual ? "Single" : "")}>
          <Input
            className={"MinValue"}
...
export default function WhereInput({
  ...
  const currentField = fields.find(
    (fieldDef) => fieldDef.name === field.replaceAll('"', ""),
  );
  let operator = ("" + value)?.includes(INTERVAL_IDENTIFIER)
    ? OPERATOR_WITH_INTERVAL
    : ("" + value)?.includes(INTERNEXT_IDENTIFIER)
      ? OPERATOR_WITH_INTERNEXT
      : where.operator;
    ...
    >
      ...
      <SelectPlaceholder
        ...
        className={
          ...
          (!Object.keys(UPDATED_OPERATOR).includes(operator) ? "Error" : "")
        }
        list={Object.keys(UPDATED_OPERATOR).map((key, idx) => {
        ...
        onChangeFn={(value) => {
          if (value === OPERATOR_WITH_INTERVAL) {
            ...
            if (
              where.value?.includes(INTERVAL_IDENTIFIER) ||
              where.value.includes(INTERNEXT_IDENTIFIER)
            ) {
              where.value = [];
            }
