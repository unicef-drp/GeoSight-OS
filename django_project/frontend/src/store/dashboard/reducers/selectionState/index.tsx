...
export const SELECTION_STATE_FILTER_ADD_INDICATOR = SELECTION_STATE + '/FILTER/ADD_INDICATOR';
export const SELECTION_STATE_FILTER_REMOVE_INDICATOR = SELECTION_STATE + '/FILTER/REMOVE_INDICATOR';
export const SELECTION_STATE_FILTER_ADD_INDICATOR_LAYER = SELECTION_STATE + '/FILTER/ADD_INDICATOR_LAYER';
export const SELECTION_STATE_FILTER_REMOVE_INDICATOR_LAYER = SELECTION_STATE + '/FILTER/REMOVE_INDICATOR_LAYER';
export const SELECTION_STATE_FILTER_ADD_RELATED_TABLE = SELECTION_STATE + '/FILTER/ADD_RELATED_TABLE';
export const SELECTION_STATE_FILTER_REMOVE_RELATED_TABLE = SELECTION_STATE + '/FILTER/REMOVE_RELATED_TABLE';
...
export default function selectedAdminLevelReducer(state = initialState, action: ActionProps) {
  if (action.name === SELECTION_STATE) {
    switch (action.type) {
      ...
      case SELECTION_STATE_FILTER_REMOVE_INDICATOR: {
        const index = state.filter.indicatorIds.findIndex((id: number) => id === action.payload);
      ...
      case SELECTION_STATE_FILTER_ADD_INDICATOR_LAYER: {
        return {
          ...
          filter: {
            ...
            indicatorLayerIds: [...state.filter.indicatorLayerIds, action.payload],
            relatedTableIds: state.filter.relatedTableIds,
          }
        }
      }
      case SELECTION_STATE_FILTER_REMOVE_INDICATOR_LAYER: {
        const index = state.filter.indicatorLayerIds.findIndex((id: number) => id === action.payload);
        if (index === -1) return state
        return {
          ...state,
          filter: {
      ...
      case SELECTION_STATE_FILTER_REMOVE_RELATED_TABLE: {
        const index = state.filter.relatedTableIds.findIndex((id: number) => id === action.payload);
        if (index === -1) return state
        return {
          ...state,
          filter: {
