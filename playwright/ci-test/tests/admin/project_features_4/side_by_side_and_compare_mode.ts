...
function setupConsoleLayerCapture(
  page: Page,
  lastLayers: Record<string, string>,
  lastLayerPaintFill: Record<string, string>,
  lastLayerPaintLine: Record<string, string>
) {
  page.on('console', msg => {
...
test.describe('Side by side and compare mode', () => {
  let lastLayers: Record<string, string>;
  let lastLayerPaintFill: Record<string, string>;
  let lastLayerPaintLine: Record<string, string>;

  test.beforeEach(async ({ page }) => {
    lastLayers = {};
    lastLayerPaintFill = {};
    lastLayerPaintLine = {};
    setupConsoleLayerCapture(page, lastLayers, lastLayerPaintFill, lastLayerPaintLine);
    await viewProject(page, 'Demo GeoSight Project');
    await page.getByRole('button', { name: 'Close' }).click();
  });

  const checkMapStyle = async (page: Page, mapIndex: number, layerName: string | null) => {
  ...
  const doSwap = async (page: Page, fromMapIndex: number, toMapIndex: number, fromLayerName: string, toLayerName: string, togglerClass: string) => {
    // Swap with MapLegendSwapIcon
    ...
    await checkMapStyle(page, fromMapIndex, toLayerName);
    await checkMapStyle(page, toMapIndex, fromLayerName);

    // Swap with MapLegendSectionTitle
    ...
    await checkMapStyle(page, fromMapIndex, fromLayerName);
    await checkMapStyle(page, toMapIndex, toLayerName);
  };
  ...
  test('Compare mode', async ({ page }) => {
    ...
    await checkMapStyle(page, 0, TEXT.INDICATOR_LAYERS.sampleIndicatorA);
    ...
    await checkMapStyle(page, 0, TEXT.INDICATOR_LAYERS.sampleIndicatorA);
    await checkMapStyle(page, 1, TEXT.INDICATOR_LAYERS.sampleIndicatorB);

    await page.getByTitle(TEXT.TOOLS_TOGGLER.sideBySide.On).click();
    await checkMapStyle(page, 0, TEXT.INDICATOR_LAYERS.sampleIndicatorA);
    await checkMapStyle(page, 1, null);
  })
  ...
  test('Side by side view', async ({ page }) => {
    ...
    await checkMapStyle(page, 0, TEXT.INDICATOR_LAYERS.sampleIndicatorA);
    await checkMapStyle(page, 1, null);
    await checkMapStyle(page, 2, null);
    await checkMapStyle(page, 3, null);
    ...
    await checkMapStyle(page, 2, null);
    await checkMapStyle(page, 3, null);
    await doSwap(page, 0, 1, TEXT.INDICATOR_LAYERS.sampleIndicatorA, TEXT.INDICATOR_LAYERS.sampleIndicatorB, "Right");

    // Turn on C
    await page.getByLabel(TEXT.INDICATOR_LAYERS.testIndicatorC).click();
    await checkMapStyle(page, 0, TEXT.INDICATOR_LAYERS.sampleIndicatorA);
    await checkMapStyle(page, 1, TEXT.INDICATOR_LAYERS.sampleIndicatorB);
    await checkMapStyle(page, 2, TEXT.INDICATOR_LAYERS.testIndicatorC);
    await checkMapStyle(page, 3, null);
    await doSwap(page, 0, 1, TEXT.INDICATOR_LAYERS.sampleIndicatorA, TEXT.INDICATOR_LAYERS.sampleIndicatorB, "RightSideTop");
    await doSwap(page, 0, 2, TEXT.INDICATOR_LAYERS.sampleIndicatorA, TEXT.INDICATOR_LAYERS.testIndicatorC, "RightSideBottom");
    await doSwap(page, 1, 2, TEXT.INDICATOR_LAYERS.sampleIndicatorB, TEXT.INDICATOR_LAYERS.testIndicatorC, "Bottom");
    ...
    await checkMapStyle(page, 1, TEXT.INDICATOR_LAYERS.sampleIndicatorB);
    await checkMapStyle(page, 2, TEXT.INDICATOR_LAYERS.testIndicatorC);
    await checkMapStyle(page, 3, TEXT.INDICATOR_LAYERS.kenyaIndicatorA);
    await doSwap(page, 0, 1, TEXT.INDICATOR_LAYERS.sampleIndicatorA, TEXT.INDICATOR_LAYERS.sampleIndicatorB, "Right");
    await doSwap(page, 0, 2, TEXT.INDICATOR_LAYERS.sampleIndicatorA, TEXT.INDICATOR_LAYERS.testIndicatorC, "BottomRight");
    await doSwap(page, 0, 3, TEXT.INDICATOR_LAYERS.sampleIndicatorA, TEXT.INDICATOR_LAYERS.kenyaIndicatorA, "Bottom");
    await doSwap(page, 1, 2, TEXT.INDICATOR_LAYERS.sampleIndicatorB, TEXT.INDICATOR_LAYERS.testIndicatorC, "Bottom");
    await doSwap(page, 1, 3, TEXT.INDICATOR_LAYERS.sampleIndicatorB, TEXT.INDICATOR_LAYERS.kenyaIndicatorA, "BottomLeft");
    await doSwap(page, 2, 3, TEXT.INDICATOR_LAYERS.testIndicatorC, TEXT.INDICATOR_LAYERS.kenyaIndicatorA, "Left");
