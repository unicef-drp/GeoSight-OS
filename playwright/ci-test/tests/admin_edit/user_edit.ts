...
test.describe('Users edit profile', () => {
  ...
  test('Test edit functions', async ({ page }) => {
    ...
    await page.locator('#Form #id_first_name').fill('Admin 1');
    await page.locator('#Form #id_last_name').fill('Admin 2');
    await page.getByRole('button', { name: 'Save' }).click();
    await expect(page.getByRole('grid')).toContainText('Admin 1');
    await expect(page.getByRole('grid')).toContainText('Admin 2');
